...
public:
    ...
    {
        return checkSameSubtree(mRoot, t.mRoot);
    }
    
    bool checkSameSubtree(Node* current, Node* other) 
    {
        if (current == nullptr && other == nullptr) return true;
        if (current == nullptr || other == nullptr) return false;
        return current->data == other->data && checkSameSubtree(current->left, other->left) && checkSameSubtree(current->right, other->right);
    }
