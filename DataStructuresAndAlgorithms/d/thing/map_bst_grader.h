#ifndef _MAP_BST_GRADER_QUEUE_INCLUDED_
#define _MAP_BST_GRADER_QUEUE_INCLUDED_
...
class map_bst
{
  ...
  public:
    ...
    void print_node(node *n,size_t depth) {
      if (n != NULL) {
        if (n->right != NULL && n->right->parent != n)
          std::cout << "error at parent of " << n->right->data.first << std::endl;
        if (n->left != NULL && n->left->parent != n)
          std::cout << "error parent of " << n->left->data.first << std::endl;
    ...
    void save_address(std::unordered_map<KeyT,void*> &result) {
      for (auto it = begin(); it != end(); it++) {
        result[it->first] = it.ptr;
      }
    }

    bool check_address(std::unordered_map<KeyT,void*> &address) {
      for (auto it = begin(); it != end(); it++) {
        auto it2 = address.find(it->first);
        if (it2 == address.end()) return false;
        //std::cout << "value = " << it->first << " saved address = " << it2->second << " my address = " <<it.ptr << std::endl;
        if (it2->second != it.ptr) return false;
      }
      return true;
    }
